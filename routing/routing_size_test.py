...
demand_size = []
supply_size = []
for problem_size in tqdm(range(11000, 21000, 1000)):
    for curr_supply_percentage in [0.05, 0.1, 0.25]:
        supply_amount_ranges = (100, 1000)
        demand_amount_ranges = (100, 500)
        area_range = (0, 10000)

        # Create our nodes
        supply_names = [f"supply_{i}" for i in range(int(problem_size*curr_supply_percentage))]
        demand_names = [f"demand_{i}" for i in range(problem_size)]

        supply_size.append(int(problem_size*curr_supply_percentage))
        demand_size.append(problem_size)
        supply_percentage.append(curr_supply_percentage)
        ...
        total_demand_adjustment = (sum(demand_amounts.values()) + 10) / sum(supply_amounts.values())

        supply_amounts = {name: supply_amounts[name] * total_demand_adjustment for name in supply_names}
        ...
        pd.DataFrame({"supply_size": supply_size, "demand_size": demand_size, "times": times,
                      "supply_percentage": supply_percentage}).\
            to_csv("runtime_by_problem_size_8_threads_w_supply_percentage_new.csv", index=False)
